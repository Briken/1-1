...
public enum ExpireOptions { Loop, Destroy, Stay };
...
public class Trickster : MonoBehaviour {
            ...
            if (m_state == States.active)
            {
                ...
                else
                {
                    ...
                    else if (m_expiringOption == ExpireOptions.Destroy)
                    {
                        ...
                        {
                            obj.SendMessage(m_messageToSend);
                        }

                        this.gameObject.SetActive(false);
                    }
                    else if (m_expiringOption == ExpireOptions.Stay)
                    {
                        //notify the objects in the array when expiring
                        foreach (GameObject obj in m_ObjectsToNotifyOnExpire)
                        {
                            if (m_messageToSend.Length != 0)
                                obj.SendMessage(m_messageToSend);
                            else
                                obj.SetActive(true);
                        }
    ...
    public void ActivateExternally()
    {
        ...
        //m_index = m_objects.Length - 1; //remove this to gradually make the object disappear or swap instead of expire
        m_state = States.active;
    }
}
